...
const Buisness_Card = () => {
  return (
    <div>
      <div className="Buisness_Card container" >
      {data.Section1.map((BuisnessCardData) => {
        return (
          <div className="row" >
            <h2 className={` text-center ${styles.sectionHeading}`}>{BuisnessCardData.Heading}</h2>
            <p className="text-center h6 mt-5 mb-5 ">{BuisnessCardData.Content}</p>
          </div>
        );
      })}
...
<div className={`row d-flex ml-100 ${styles.Img_Cards} `} >
      ...
      <div className={`text-center mt-5`}>
        <p ><a   className={`p-2 ${styles.Explorebtn}`} href="">Explore More</a></p>
      </div>
    </div>
    </div>
  );
